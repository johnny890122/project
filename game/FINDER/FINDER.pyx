...
#################################################New code for FINDER#####################################
    def BuildNet(self):
        # [2, embed_dim]
        w_n2l = tf.Variable(tf.compat.v1.random.truncated_normal([2, self.embedding_size], stddev=initialization_stddev), tf.float32)
        # [embed_dim, embed_dim]
        p_node_conv = tf.Variable(tf.compat.v1.random.truncated_normal([self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)
        if embeddingMethod == 1:    #'graphsage'
            # [embed_dim, embed_dim]
            p_node_conv2 = tf.Variable(tf.compat.v1.random.truncated_normal([self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)
            # [2*embed_dim, embed_dim]
            p_node_conv3 = tf.Variable(tf.compat.v1.random.truncated_normal([2*self.embedding_size, self.embedding_size], stddev=initialization_stddev), tf.float32)
        ...
        #    h1_weight = tf.Variable(tf.truncated_normal([2 * self.embedding_size, self.reg_hidden], stddev=initialization_stddev), tf.float32)
            ...
            h1_weight = tf.Variable(tf.compat.v1.random.truncated_normal([self.embedding_size, self.reg_hidden], stddev=initialization_stddev), tf.float32)
            ...
            #[reg_hidden+aux_dim, 1]
            h2_weight = tf.Variable(tf.compat.v1.random.truncated_normal([self.reg_hidden + aux_dim, 1], stddev=initialization_stddev), tf.float32)
            #[reg_hidden2 + aux_dim, 1]
            last_w = h2_weight
        else:
            #[2*embed_dim, reg_hidden]
            h1_weight = tf.Variable(tf.compat.v1.random.truncated_normal([2 * self.embedding_size, self.reg_hidden], stddev=initialization_stddev), tf.float32)
            # [embed_dim, reg_hidden]
        ...
        ## [embed_dim, 1]
        cross_product = tf.Variable(tf.compat.v1.random.truncated_normal([self.embedding_size, 1], stddev=initialization_stddev), tf.float32)
        ...
        while lv < max_bp_iter:
            ...
            n2npool = tf.sparse.sparse_dense_matmul(tf.cast(self.n2nsum_param,tf.float32), cur_message_layer)
            ...
            y_n2npool = tf.sparse.sparse_dense_matmul(tf.cast(self.subgsum_param,tf.float32), cur_message_layer)
       ...
       # y_potential = tf.sparse_tensor_dense_matmul(tf.cast(self.subgsum_param,tf.float32), cur_message_layer)
        ...
        #[batch_size, node_cnt] * [node_cnt, embed_dim] = [batch_size, embed_dim]
        action_embed = tf.sparse.sparse_dense_matmul(tf.cast(self.action_select, tf.float32), cur_message_layer)
      ...
      #  embed_s_a = tf.concat([action_embed,y_potential],1)
       ...
       # [batch_size, embed_dim], first transform
        ...
        loss_recons = 2 * tf.linalg.trace(tf.matmul(tf.transpose(cur_message_layer), tf.sparse.sparse_dense_matmul(tf.cast(self.laplacian_param,tf.float32), cur_message_layer)))
        edge_num = tf.compat.v1.sparse_reduce_sum(tf.cast(self.n2nsum_param, tf.float32))
      ...
      #  embed_s_a_all = tf.concat([cur_message_layer,rep_y],1)
        ...
        rep_aux = tf.sparse.sparse_dense_matmul(tf.cast(self.rep_global, tf.float32), self.aux_input)
    ...
    def GetSol(self, int gid, int step=1):
        ...
        nodes = list(range(g_list[0].num_nodes))
        solution = list(set(nodes)^set(sol))
